...
def DelphesWithTruthToHDF5(delphes_files, f_truth_files=None, truth_files=None, h5_file=None, nentries_per_chunk=1e4, verbosity=0):
    ...
    for i in range(nchunks):
        ...
        f_truth_particles = [
            ev.particles[:int(np.minimum(len(ev.particles),f_n_truth = 1e4))]
        ...
        for j in range(ranges[i]):
            ...
            vecs = np.concatenate([truth_particles[j][k].momentum.px,
                                       truth_particles[j][k].momentum.py,truth_particles[j][k].momentum.pz,
                                       truth_particles[j][k].momentum.e] for k in range(f_truth_particles[j]))
            ...
            # Get the constituents of our selected jet. Assuming they are massless -> don't need to fetch the mass.
            pt,eta,phi, m = np.hsplit(np.array([[x.pt(), x.eta(), x.phi(), x.m()] for x in jet.constituents()]),3)
            pt = pt.flatten()
            eta = eta.flatten()
            phi = phi.flatten()
            m = m.flatten()
