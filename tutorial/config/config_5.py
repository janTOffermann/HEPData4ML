...
config = {
    ...
    'pileup' : {
        'handler': pu.PileupOverlayPtFilter(
            "output/tutorial_0/events*.root",
            rng_seed=1,
            precompute=True
        ) # This pileup handler will run FastJet to compute the leading jet pT for each event in the pileup input. With precompute=True, this is done all at the start; it will also write the results to the input HepMC3/ROOT files themselves, for fast lookup next time.
    },
