...
Bool_t DelphesHepMC3ROOTReader::ReadEvent(){
  // if(fEvent != 0) fEvent->clear(); // probably unnecessary
  fReader->read_event(fEvent);

  Bool_t status = !fReader->failed();

  // if(status){ // will analyze event
  // }

  return status;
}

bool DelphesHepMC3ROOTReader::ReadBlock(DelphesFactory *factory,
  TObjArray *allParticleOutputArray,
  TObjArray *stableParticleOutputArray,
  TObjArray *partonOutputArray)
{

  // In DelphesHepMC3Reader, this read a "DelphesStream" to access
  // the plaintext HepMC3 file. Kind of hard to read, lots of if statements...
  // Here, we just need to read in an event
...
void DelphesHepMC3ROOTReader::AnalyzeParticle(DelphesFactory *factory, std::shared_ptr<HepMC3::GenParticle> particle)
{
  Candidate *candidate;
