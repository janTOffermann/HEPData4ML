...
class PythiaWrapperSetup:
    ...
    def SetDirectory(self,dir):
        if(dir is None):
            dir = os.path.dirname(os.path.abspath(__file__)) + '/../root/pythia_generator'
        self.dir = dir
    ...
    def Build(self):
        """
        Builds the PythiaGenerator C++/ROOT library.
        ...
        self.run_command_with_progress(command,cwd=self.dir,prefix='Building PythiaGenerator:',output_width=80)
        return

    def Load(self,quiet=False):
        # Load our custom ROOT library.
        try:
            a = rt.PythiaGenerator
            return
        except:
            pass
        ...
        build_dir = self.dir + '/pythia_generator/build'
        include_dir = self.dir + '/pythia_generator/inc'
        custom_lib_paths = [os.path.realpath('{}/lib/libPythiaGenerator.{}'.format(build_dir,x)) for x in self.lib_extensions]
        ...
        if(not found_libary):
            if(not quiet): print('Error: The PythiaGenerator lib has not been built!')
            assert False
        ...
        assert status == 0, 'The PythiaGenerator lib did not load properly.'
        return
