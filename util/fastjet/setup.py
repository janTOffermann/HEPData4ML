...
class FastJetSetup:
    ...
    # def run_make_with_progress_by_lines(self, command=['make'], prefix='Building',
    #                                    expected_lines=1000, show_output_lines=10, output_width=80):
    #     """
    #     Run make command with progress bar based on line count and scrolling output display

    #     Args:
    #         command: Command to run (default: ['make'])
    #         prefix: Progress bar prefix
    #         expected_lines: Expected total number of output lines for progress calculation
    #         show_output_lines: Number of recent output lines to show (0 to disable)
    #         output_width: Width of output display area
    #     """
    ...
    #         # Initialize display area
    #         if show_output_lines > 0:
    #             # Reserve space for progress bar + bordered output area
    #             for i in range(1 + show_output_lines + 2):  # +2 for top and bottom border
    #                 print()
    #             # Initial display
    #             printProgressWithOutput(current_progress, 100, recent_lines,
    #                                   prefix=prefix, suffix='Complete',
    ...
    #                 try:
    #                     # Get line with timeout to check process status
    #                     _, line = line_queue.get(timeout=0.1)

    #                     # Increment line counter and calculate progress
    #                     line_counter += 1
    #                     progress = (line_counter / expected_lines) * 100.0
    #                     current_progress = int(np.minimum(progress, 100))  # Cap at 100%
    ...
    #         # Final progress update - always show 100% when complete
    #         if show_output_lines > 0:
    #             printProgressWithOutput(100, 100, recent_lines,
    #                                   prefix=prefix, suffix='Complete',
    #                                   max_lines=show_output_lines, width=output_width)
    ...
    #     # Return actual line count for reference/calibration
    #     return line_counter