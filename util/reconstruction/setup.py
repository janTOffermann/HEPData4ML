...
class NtupleProducerSetup:
    """
    This class is for setting up the C++/ROOT NtupleProduceer code.
    This allows for producing ROOT ntuple with some C++ code, which
    is faster than doing things entirely in Python.
    """
    def __init__(self,ntuple_dir=None):
        self.SetDirectory(ntuple_dir)
    ...
    def SetDirectory(self,dir):
        if(dir is None):
            dir = os.path.dirname(os.path.abspath(__file__)) + '/../root/ntuple'
        self.dir = dir
    ...
    def Build(self):
        """
        Builds the NtupleProducer C++/ROOT library.
        ...
        self.run_command_with_progress(command,cwd=self.dir,prefix='Building NtupleProducer:',output_width=80)
        return
    ...
    def Load(self,quiet=False):
        ...
        build_dir = self.dir + '/ntuple/build'
        include_dir = self.dir + '/ntuple/inc'
        custom_lib_paths = [os.path.realpath('{}/lib/libNtupleProducer.{}'.format(build_dir,x)) for x in self.lib_extensions]
        ...
        if(not found_libary):
            if(not quiet): print('Error: The NtupleProducer lib has not been built!')
            assert False
        ...
        assert status == 0, 'The NtupleProducer lib did not load properly.'
        return
