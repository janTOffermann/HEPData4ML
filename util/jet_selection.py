...
class GetNearestLeadingJet:
    ...
    def __call__(self,**kwargs):
        ...
        # Now find distance between the truth particle and each of the jets.
        # Within the dR window, return the highest-pT jet.
        dr2 = np.array([self.calculator.DeltaR2(truth_eta,truth_phi,x.eta(),x.phi()) for x in jets]).flatten()
        close_indices = np.array(np.where(dr2 < self.max_dr * self.max_dr)[0],dtype=int)
        if(len(close_indices) == 0): return -1
        jet_pt = np.array([x.pt() for x in jets])[close_indices]
        return close_indices[np.argmax(jet_pt)]
