...
config = {
'proc' : 'Top_Semileptonic_2', # Name must correspond to a card name (without extension) in util/pythia_templates
...
'delphes_dir' : '/cvmfs/sft.cern.ch/lcg/releases/delphes/3.5.1pre09-812da/x86_64-el9-gcc11-opt' ,# directory containing the Delphes installation. If None, will be build in a local directory "delphes".
...
'jet_n_par': 1000, # max number of jet constituents to save per jet (pT-ordered)
...
#     eventfilter.MultiFilter( # multi-filter -- applies filters (cuts) sequentially
#     [
#         eventfilter.ContainedJetFilter(0.8,selections['t->Wb'],selections['b daughters'],pt_threshold_sum=2.5,pt_min_jet=15.,eta_max_jet=2.,matching_radius=0.8), # require stable W daughters within jet -- allow sum of uncontained daughters to carry off up to 2.5 GeV in pT
#         eventfilter.ContainedJetFilter(0.8,selections['t->Wb'],selections['W daughters'],pt_threshold_sum=2.5,pt_min_jet=15.,eta_max_jet=2.,matching_radius=0.8), # require stable b daughters within jet -- allow sum of uncontained daughters to carry off up to 2.5 GeV in pT
#     ]
#     ),
# name = 'jet_top_daughters_contained_dR08'
# ),
'truth_selection' : selections['t->Wb w/ qq and W daughters'],
'final_state_selection': parsel.AlgoSelection(algos.SelectFinalState(),1000),
...
'record_final_state_indices' : False, # Whether or not to record jet constituents' indices w.r.t. the order they were passed to jet clustering (order of particles in HepMC file, or order of Delphes objects if using Delphes).
