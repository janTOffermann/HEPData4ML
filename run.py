...
def main(args):
    ...
    for i in range(nbins):
        ...
        hepfile = 'events_{}-{}.hepmc'.format(pt_min,pt_max)
        ...
        delete_hepmc = True
        comm = ['tar','-cjf',hepfile.replace('.hepmc','.tar.bz2'),hepfile]
        if(delete_hepmc): comm.append('--remove-files')
        sub.check_call(comm,shell=False)
    ...
    for truth_file in truth_files:
        comm = ['tar','-cjf',truth_file.replace('.hepmc','.tar.bz2'),truth_file]
        if(delete_hepmc): comm.append('--remove-files')
        sub.check_call(comm)
